...
Future<Ui_hotel_meal> create_thisPage_Model(
  ...
  final response = await http.post(
    ...
    body: jsonEncode({
      "action": "show_hotel_meal_ui_data",
  ...
  if (response.statusCode == 200) {
    ...
    Ui_hotel_meal hotelUi = Ui_hotel_meal.fromJson(jsonD['dataView']);

    return hotelUi;
  } else {
    // If the server did not return a 200 CREATED response,
    // then throw an exception.
    throw Exception('Failed to create hotel_meal_ui .');
  }
}

class Newhotel_meal extends StatefulWidget {
  final int hotel_reservation_id;
  const Newhotel_meal({Key? key, required this.hotel_reservation_id})
      : super(key: key);

  @override
  State<Newhotel_meal> createState() {
    return _Newhotel_mealState(hotel_reservation_id: hotel_reservation_id);
  }
}

class _Newhotel_mealState extends State<Newhotel_meal> {
  final TextEditingController _controller = TextEditingController();

  final int hotel_reservation_id;
  _Newhotel_mealState({required this.hotel_reservation_id});
  ...
  FutureBuilder<Ui_hotel_meal> buildFutureBuilder() {
    return FutureBuilder<Ui_hotel_meal>(
      future: create_thisPage_Model(hotel_reservation_id),
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          Ui_hotel_meal hotelUi = snapshot.data!;
          return Newhotel_mealPage(hotelUi,
...
class Newhotel_mealPage extends StatefulWidget {
  final int hotel_reservation_id;
  Ui_hotel_meal hotel_ui;
  Newhotel_mealPage(this.hotel_ui,
      {Key? key, required this.hotel_reservation_id})
      : super(key: key);

  @override
  _Newhotel_mealPage createState() =>
      _Newhotel_mealPage(hotel_ui, hotel_reservation_id: hotel_reservation_id);
}

class _Newhotel_mealPage extends State<Newhotel_mealPage> {
  final int hotel_reservation_id;
  Ui_hotel_meal hotel_ui;

  _Newhotel_mealPage(this.hotel_ui, {required this.hotel_reservation_id});
  ...
  @override
  Widget build(BuildContext context) {
    _hotel_namesJSON = hotel_ui.hotel_meal_names;
              ...
              : Center(
                  child: FormHelper.submitButton(
                    ...
                    () async {
                      ...
                      if (_c_meal_id == "" || _c_meal_id == null) {
                        showAlertDialog(
                            context, "Check meal_id!", "Your meal_id is empty");
                        return;
                      }

                      Map<String, dynamic> responseList =
                          await api_connect_post_request({
                        "action": "new_hotel_meal",
